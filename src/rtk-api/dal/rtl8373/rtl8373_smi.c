...
extern unsigned int mii_mgr_write(u32 phy_addr, u32 phy_register, u32 write_data);
extern unsigned int mii_mgr_read(u32 phy_addr, u32 phy_register, u32 *read_data);

/* MDC/MDIO, redefine/implement the following Macro */
#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data) mii_mgr_write(phyID, regID, (u32)data)
#define MDC_MDIO_READ(preamableLength, phyID, regID, pData) mii_mgr_read(phyID, regID, (u32*)pData)
...
extern struct mutex rtl_mii_lock;

static void rtlglue_drvMutexLock(void)
{
    mutex_lock(&rtl_mii_lock);
    return;
}

static void rtlglue_drvMutexUnlock(void)
{
    mutex_unlock(&rtl_mii_lock);
    return;
}
...
rtk_int32 rtl8373_smi_read(rtk_uint32 mAddrs, rtk_uint32 *rData)
{
    ...
    /*Ckeck busy bit, register21, bit2*/
    MDC_MDIO_READ(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID,MDC_MDIO_CTRL_REG, &regData);
    ...
    /*write reg address, register22*/
    MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_ADDR_REG, mAddrs);

    /* Write read command   register21 */
    MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL_REG, MDC_MDIO_READ_CMD);
    ...
    /* Write read control code to register 23 */
    MDC_MDIO_READ(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_LOW, &datal);

    /* Read data from register 24 */
    MDC_MDIO_READ(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_HIGH, &datah);

    *rData = datal & 0xffff;
    *rData |= (datah & 0xffff) << 16;
...
rtk_int32 rtl8373_smi_write(rtk_uint32 mAddrs, rtk_uint32 rData)
{
    ...
    /*write low 16bits data, register23*/
    datal = rData & 0xffff;
    MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_LOW, datal);

    /*write high 16bits data, register24*/
    datah = (rData >> 16) & 0xffff;
    MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_HIGH, datah);

    /*write  write cmd,  register 21*/
    MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL_REG, MDC_MDIO_WRITE_CMD);
